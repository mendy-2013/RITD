...
class LVCBlock(nn.Module):
    def __init__(self,in_channels, out_channels, channel_ratio=0.25, base_channel=64):
        super(LVCBlock, self).__init__()
        self.in_channels = in_channels
        self.out_channels = out_channels
        self.num_codes = 64

        self.conv_1 = ConvBlock(in_channels=self.in_channels, out_channels=self.in_channels, res_conv=True, stride=1)

        self.LVC = nn.Sequential(
            nn.Conv2d(self.in_channels, self.in_channels, 1, bias=False),
            nn.BatchNorm2d(self.in_channels),
            nn.ReLU(inplace=True),
            Encoding(in_channels=self.in_channels, num_codes=self.num_codes),
            nn.BatchNorm1d(self.num_codes),
            nn.ReLU(inplace=True),
            Mean(dim=1))
        self.fc = nn.Sequential(nn.Linear(self.in_channels, self.in_channels), nn.Sigmoid())
