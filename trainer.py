    ...
    def train(self):
        ...
        while True:
            ...
            for batch in train_data_loader:
                # data = batch['mask'].to(self.device)
                # a = torch.sum(data == 1).item()
                # a0 = torch.sum(data == 0).item()
                # gt = batch['gt'].to(self.device)
                # c = torch.sum(gt == 1).item()
                # c0 = torch.sum(gt == 0).item()
                # aux_data = batch["thresh_mask"].to(self.device)
                # b = torch.sum(aux_data == 1).item()
                # b0 = torch.sum(aux_data == 0).item()
