...
from .spin import GenSP, SPInterAttModule, FFN, SPIntraAttModule, LocalTokenAttention
...
class SegSpatialScaleDetector(nn.Module):
    def __init__(self,
        ...
        self.intra_layer = nn.ModuleList([
            SPIntraAttModule(dim, heads, qk_dim, topk=(16 ** 2)),
            FFN(dim, mlp_dim, dim),
        ])
        self.local_layer = nn.ModuleList([
            LocalTokenAttention(dim, heads, qk_dim, [16, 16]),
            FFN(dim, mlp_dim, dim),
        ])
        ######
    ...
    # def forward(self, features, gt=None, masks=None, training=False):
        ...
        if self.training:
            ...
            # affinity_matrix, num_spixels = self.gen_super_pixel(fuse_out, self.stoken_size)
            # fuse_out = inter_attn(fuse_out, affinity_matrix, num_spixels) + fuse_out
            # fuse_out = inter_ff(fuse_out) + fuse_out
            #
            # affinity_matrix, num_spixels = self.gen_super_pixel(fuse_out, self.stoken_size)
            # fuse_out = inter_attn(fuse_out, affinity_matrix, num_spixels) + fuse_out
            ...
            # fuse_out = self.conv3x1(fuse_out)

            intra_attn, intra_ff = self.intra_layer
            fuse_out = intra_attn(fuse_out, affinity_matrix, num_spixels) + fuse_out
            fuse_out = intra_ff(fuse_out) + fuse_out

            local_attn, local_ff = self.local_layer
            fuse_out = local_attn(fuse_out) + fuse_out
            fuse_out = local_ff(fuse_out) + fuse_out
        ...
        if self.adaptive and self.training:
            ...
            # result.update(thresh=thresh, thresh_binary=thresh_binary, aux_gdsc=aux, focus=fc)
