...
class L1BalanceCELoss(nn.Module):
    ...
    def __init__(self, eps=1e-6, l1_scale=10, bce_scale=5):
        ...
        from .dice_loss import DiceLoss
        from .dice_loss import DiceLoss1   ###
        from .l1_loss import MaskL1Loss
        from .balance_cross_entropy_loss import BalanceCrossEntropyLoss
        self.dice_loss = DiceLoss(eps=eps)
        self.dice_loss1 = DiceLoss1(eps=eps)  ###
        self.l1_loss = MaskL1Loss()
    ...
    def forward(self, pred, batch):
        ...
        if 'thresh' in pred:
            ...
            #### ferine ####
            # dice_loss_rf = self.dice_loss1(pred['refine'], batch['gt'], batch['mask'])
            # metrics["refine_loss"] = dice_loss_rf


            #### gdsc_mask ####
            dice_loss_gdsc = self.dice_loss1(pred['aux_gdsc'], batch['gt'], batch['mask'])
            metrics["aux_loss"] = dice_loss_gdsc

            #### super_pixel ####
            # dice_loss_super = self.dice_loss1(pred['super_pixel'], batch['gt'], batch['mask'])
            # metrics["super_loss"] = dice_loss_super
            ##
            # ## gdsc_focus
            # bce_loss_focus = self.bce_loss(pred['focus'], batch['gt'], batch['mask'])
            # metrics["focus_loss"] = bce_loss_focus
            # ##
            # ## clip
            # # pred['text_1'] = pred['text_1'].repeat(16, 1)
            # # pred['text_1'] = F.normalize( pred['text_1'],dim=-1,p=2)
            # # pred['text_0'] = pred['text_0'].repeat(16, 1)
            # # pred['text_0'] = F.normalize(pred['text_0'], dim=-1, p=2)
            # # pred['text_tensor'] = F.normalize(pred['text_tensor'], dim=-1, p=2)
            # a = pred['binary'].float()
            # a = F.interpolate(a,size=(512,512), mode='bilinear', align_corners=False)
            # a = a.sum(dim=(1, 2))
            # b = pred['text_1'].repeat(16, 1).float()
            # c = pred['text_0'].repeat(16, 1).float()
            # # Compute cosine similarity
            # sim_ab = F.cosine_similarity(a, b, dim=-1)
            # sim_ac = F.cosine_similarity(a, c, dim=-1)
            # # Compute numerator and denominator for InfoNCE loss
            # exp_sim_ab = torch.exp(sim_ab / 1)
            # exp_sim_ac = torch.exp(sim_ac / 1)
            # # Compute InfoNCE loss
            # triplet_loss = -torch.log(exp_sim_ab / (exp_sim_ab+exp_sim_ac))
            # triplet_loss = torch.mean(triplet_loss)
            # metrics["clip_loss"] = triplet_loss
            # ###
            metrics['thresh_loss'] = dice_loss
            loss = dice_loss + self.l1_scale * l1_loss + bce_loss * self.bce_scale + 15 * dice_loss_gdsc
                   #+ 10 * dice_loss_super + 15 * dice_loss_gdsc
            metrics.update(**l1_metric)
        else:
            loss = bce_loss
        return loss, metrics
