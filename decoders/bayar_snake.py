...
#--------------------------------------------bayar_snake_conv_block--------------------------------------------#
class Bayar_Snake_block(nn.Module):
    # bayar conv and snake conv with two convolution
    def __init__(self, in_channels, out_channels, kernel_size=3, stride = 1, padding = None, act = True):
        super(Bayar_Snake_block,self).__init__()
        self.in_channels = in_channels
        self.out_channels = out_channels
        self.kernel_size = kernel_size
        self.stride = stride
        self.padding = padding
        self.mid = int(out_channels // 2)    # out
        self.act = nn.SiLU() if act is True else (act if isinstance(act, nn.Module) else nn.Identity())

        # first standard conv
        self.conv1 = nn.Conv2d(in_channels, self.mid, kernel_size, stride, autopad(kernel_size, padding), groups=1, bias=False)
        self.bn1 = nn.BatchNorm2d(self.mid)
        self.act = nn.SiLU() if act is True else (act if isinstance(act, nn.Module) else nn.Identity())

        # bayar branch
        self.bayar = BayarConv2d(self.mid,self.mid)

        # snake branch
        self.snake = DSConv(self.mid,
                            self.mid,      #偶数(//4) 对4取整
                            kernel_size=5,
                            extend_scope=1,
                            morph=0,
    ...
    def forward(self, x):
        ...
        x = self.act(x)
        identify = x                # residual 
        # print("identify {}".format(identify.shape))
        bayar = self.bayar(x)       # bayar branch
        # print("bayar {}".format(bayar.shape))
        snake = self.snake(x)       # snake branch
        # print("snake {}".format(snake.shape))
        x = bayar + identify + snake
        x = self.conv2(x)
        x = self.bn2(x)
        x = self.act(x)
        # print("x {}".format(x.shape))

        # return  torch.cat((x,identify),dim=1)
        return  x
#--------------------------------------------bayar_snake_conv_block--------------------------------------------#
...
if __name__ == "__main__":
    import time
    start = time.time()
    input_x = torch.randn((1,28,32,32))
    input_x = input_x.to(device)
    print("input_x  {}".format(input_x .shape))
    model = Bayar_Snake_block(28,16)    # out_channel
    model = model.cuda()
    print(model)
